...
import { initializeFileUploads, addMagicUIToFileUploads, uploadLogFile, updateFileUploadText, uploadGcpCredentials } from './components/file-upload.js';
import { initializeModeSwitch, addMagicUIToModeSwitch } from './components/mode-switch.js';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    progressBar.innerHTML = '<div id="upload-progress-inner" style="height:100%;width:0;background:#10b981;border-radius:4px;transition:width 0.2s;"></div>';
    ...
    simulationLogsInput.addEventListener('change', async function() {
        ...
        if (simulationLogsInput.files.length > 0) {
            ...
            try {
                ...
                await uploadLogFile(file, (percent) => {
    ...
    const gcpCredentialsUploadArea = document.getElementById('gcp-credentials-upload-area');
    if (gcpCredentialsUploadArea) {
        ...
        if (!gcpProgressBar) {
            ...
            gcpProgressBar.innerHTML = '<div id="gcp-upload-progress-inner" style="height:100%;width:0;background:#10b981;border-radius:4px;transition:width 0.2s;"></div>';
    ...
    // --- REWRITTEN GCP Credentials Upload Logic ---
    console.log('GCP upload logic loaded');
    ...
    let gcpServiceAccountFile = null;
    ...
    if (gcpUploadArea && gcpFileInput) {
        ...
        gcpFileInput.addEventListener('change', async function() {
            if (gcpFileInput.files.length > 0) {
                gcpServiceAccountFile = gcpFileInput.files[0];
                gcpFilenameSpan.textContent = 'File selected: ' + gcpServiceAccountFile.name;
                gcpFilenameSpan.style.color = '#10b981';
                console.log('Selected file:', gcpServiceAccountFile);
                // Immediately POST to backend for validation
                const formData = new FormData();
                formData.append('service_account_file', gcpServiceAccountFile);
                try {
                    ...
                    if (data.success) {
                        gcpFilenameSpan.textContent += ' (Credentials valid)';
                        gcpFilenameSpan.style.color = '#10b981';
                    } else {
                        gcpFilenameSpan.textContent += ' (Invalid credentials)';
                        gcpFilenameSpan.style.color = '#dc2626';
                    }
                } catch (err) {
                    gcpFilenameSpan.textContent += ' (Validation error)';
                    gcpFilenameSpan.style.color = '#dc2626';
                    console.log('Validation error:', err);
                }
            } else {
                gcpServiceAccountFile = null;
        ...
        gcpFileInput.addEventListener('change', async function() {
            if (gcpFileInput.files.length > 0) {
                ...
                try {
                    ...
                    const data = await resp.json();
                    if (!resp.ok || !data.success) throw new Error(data.message || 'Invalid credentials');
                    if (gcpUploadStatus) {
                        gcpUploadStatus.textContent = 'Credentials valid. Fetching projects...';
                    ...
                    if (!projectsResp.ok || !projectsData.success) throw new Error(projectsData.message || 'Failed to list projects');
                    // Populate project ID dropdown
                    projectIdSelect.innerHTML = '<option value="" disabled selected>Select a project</option>';
                    ...
                    resourceTypeSelect.innerHTML = '<option value="" disabled selected>Select resource type</option>';
                    resourceTypeSelect.disabled = true;
                    severitySelect.innerHTML = '<option value="" disabled selected>Select severity</option>';
        ...
        projectIdSelect.addEventListener('change', async function() {
            ...
            try {
                ...
                if (!resp.ok || !data.success) {
                    gcpUploadStatus.textContent = data.message || 'Failed to get log metadata';
                    gcpUploadStatus.style.color = '#dc2626';
                    resourceTypeSelect.innerHTML = '<option value="" disabled selected>Select resource type</option>';
                    resourceTypeSelect.disabled = true;
                    severitySelect.innerHTML = '<option value="" disabled selected>Select severity</option>';
    ...
    if (setupSubmitBtn) {
        setupSubmitBtn.addEventListener('click', async function(e) {
            ...
            const isSimulationMode = modeToggle && modeToggle.checked;
            const mode = isSimulationMode ? 'simulation' : 'live';
            const monitoringStatus = document.getElementById('monitoring-status-message');
            ...
            if (!isSimulationMode) {
                ...
                const gcpUploadStatus = document.getElementById('gcp-upload-status');
                const gcpProgressBar = document.getElementById('gcp-upload-progress-bar');
                const gcpProgressInner = document.getElementById('gcp-upload-progress-inner');
                setupSubmitBtn.disabled = true;
                if (!gcpFileInput || gcpFileInput.files.length === 0) {
                    monitoringStatus.textContent = 'Please select a service account JSON file.';
                ...
                try {
                    ...
                    await uploadGcpCredentials(
                        ...
                        (percent) => {
                            if (gcpProgressInner) gcpProgressInner.style.width = percent + '%';
                        },
                        mode
                    );
                    if (gcpUploadStatus) {
                        gcpUploadStatus.textContent = 'Logs fetched and ingested successfully!';
                        gcpUploadStatus.style.color = '#10b981';
                    }
                } catch (err) {
                    monitoringStatus.textContent = 'GCP log ingestion failed: ' + (err.message || err);
                    monitoringStatus.style.color = '#dc2626';
                    setupSubmitBtn.disabled = false;
                ...
                if (!window.simulationLogUploaded) {
                    monitoringStatus.textContent = 'Please upload a log file for simulation.';
                    monitoringStatus.style.color = '#dc2626';
                    return;
                }
                monitoringStatus.textContent = 'Running anomaly detection...';
    ...
    async function runMonitoringPipeline(mode) {
        const monitoringStatus = document.getElementById('monitoring-status-message');
        monitoringStatus.innerHTML = '<strong>Running RCA analysis...</strong><br><div id="rca-reports"></div>';
        monitoringStatus.style.color = '#374151';
